<template>
  <v-row :id="id" v-bind="getRow" v-resize.quiet="onResize">
    ...
    <!-- <pre>{{flatCombinedArraySorted}}</pre> -->
    <fragment v-for="(obj, index) in flatCombinedArraySorted" :key="index">
      <!-- Tooltip Wrapper -->
      <v-tooltip
                ...
                <template v-else-if="obj.schema.type === list">
                  <v-list>
                    ...
                    <v-list-item-group
                      ...
                      light
                    >
                      <fragment v-for="(item, idx) in setValue(obj)" :key="idx">
                        <v-list-item @click="onEvent($event, obj, list)">
                          <slot
                            :name="getArrayItemSlot(obj)"
                            v-bind="{ obj, id, index, idx, item }"
                          >
...
export default {
  ...
  methods: {
    isVisible(obj) {
      const visible = obj.schema.visible;
      if (typeof visible === "function" || visible instanceof Function) {
        return visible(this.valueIntern, obj.schema);
    ...
    autogenerateSchema(value) {
      ...
      let schema = JSON.stringify(value, (_key, val) =>
